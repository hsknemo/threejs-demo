...
import cubeNineLayer from "./model/cubeNine/cubeNineLayer.js";
window.camera = null;
window.scene = null;
...
function init() {
  ...
  // 将相机放到立方体内部，可以看到所有六个面
  camera.position.set(55, 60, 60);
  camera.lookAt(0, 0, 0);
...
function initSceneMesh(scene) {
  window.sceneConfig.cubeNineLayer.layout(scene);
}
...
function animate() {
  ...
  controls.update();
  // 更新所有面的扩散状态
  window.sceneConfig.cubeNineLayer.update();
